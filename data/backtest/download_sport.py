#!/usr/bin/env python3
"""Generic sport timeseries downloader. Usage: python3 download_sport.py <prefix> e.g. lol, val, cs2, dota2, cbb, nba"""
import json, asyncio, aiohttp, os, sys, time
...
OUT_FILE = f"{BASE}/timeseries_all.jsonl"
PROGRESS_FILE = f"{BASE}/download_progress.json"
CONCURRENCY = 10
...
with open(META_FILE) as f:
    for line in f:
        d = json.loads(line)
        if not d.get('slug','').startswith(f'{SPORT}-'): continue
...
print(f"[{SPORT}] Remaining: {len(markets)} (already done: {len(done_slugs)})", flush=True)
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        with open(OUT_FILE, 'a') as out:
            for batch_start in range(0, len(markets), batch_size):
                ...
                print(f"  [{SPORT}][{total_done}] batch done, last={batch[-1]['slug']}", flush=True)

asyncio.run(main())
print(f"[{SPORT}] Done!", flush=True)
