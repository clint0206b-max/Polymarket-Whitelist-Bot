 ...
 * Config: min_entry_price_cs2=0.87, max_entry_price_cs2=0.93, max_spread_cs2=0.02
 * SL: stop_loss_bid_cs2=0.40, stop_loss_ask_cs2=0.45
 */
...
describe("CS2 SL config values in local.json", () => {
  it("stop_loss_bid_cs2=0.40", async () => {
    const { readFileSync } = await import("node:fs");
    const c = JSON.parse(readFileSync("src/config/local.json", "utf8"));
    assert.equal(c.paper.stop_loss_bid_cs2, 0.40);
  });

  it("stop_loss_ask_cs2=0.45", async () => {
    const { readFileSync } = await import("node:fs");
    const c = JSON.parse(readFileSync("src/config/local.json", "utf8"));
    assert.equal(c.paper.stop_loss_ask_cs2, 0.45);
  });

  it("min_entry_price_cs2=0.87", async () => {
    const { readFileSync } = await import("node:fs");
    const c = JSON.parse(readFileSync("src/config/local.json", "utf8"));
    assert.equal(c.filters.min_entry_price_cs2, 0.87);
  });
